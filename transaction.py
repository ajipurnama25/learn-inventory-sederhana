...
def read_stock():
    range_name = 'stock barang!A2:H'  # Read data from the "stock barang" sheet
...
def read_price():
    range_name = 'harga barang!A2:D'  # Read data from the "harga barang" sheet
    spreadsheet_id = '10YgWzGWXBtVvlLRnPyZAZ-xmt0cNZZwRu_23wgqyccw'
    result = spreadsheet_service.spreadsheets().values().get(
        spreadsheetId=spreadsheet_id, range=range_name).execute()
    rows = result.get('values', [])
...
def update_stock(new_stock):
    range_name = 'stock barang!A2:C'  # Update data in the "stock barang" sheet
    spreadsheet_id = '10YgWzGWXBtVvlLRnPyZAZ-xmt0cNZZwRu_23wgqyccw'
